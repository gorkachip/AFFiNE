...
export function parseValue(raw: string | undefined): ActivityLogValue {
...
export function serializeValue(value: ActivityLogValue): string {
...
// Same normalization used by both insertMention (writes the handle) and
// extractMentions / renderTextWithMentions (reads it back). Spaces in a
// member name become dots in the inserted handle.
function memberHandle(m: {
  name?: string | null;
  email?: string | null;
  id: string;
}): string {
  if (m.name) return m.name.replace(/\s+/g, '.').toLowerCase();
  if (m.email) return m.email.split('@')[0].toLowerCase();
...
export function extractMentions(
  ...
  while ((m = re.exec(text)) !== null) {
    ...
    const member = members.find(mb => memberHandle(mb) === handle);
...
interface UseActivityLogResult {
  parsed: ActivityLogValue;
  entryCount: number;
  lastEntry: ActivityEntry | undefined;
...
export function useActivityLogPanel(
  ...
  const triggerSnippet = lastEntry
    ? `${lastEntry.authorName}: ${lastEntry.text.slice(0, 60)}${
        lastEntry.text.length > 60 ? '…' : ''
      }`
    : 'No entries yet';
  ...
  const renderEntry = (entry: ActivityEntry, isReply = false): ReactNode => {
